...
5.  Genere una clave SSH y agregue a GitHub (opcional pero altamente recomendado). Se recomienda utilizar autenticación mediante claves SSH, ya que evita introducir credenciales cada vez que se envían cambios al repositorio.
...
8.  Abra una terminal nueva y ejecute las siguientes líneas una por una:
    ...
    **Identidad**
    ...
    `git config --global user.email "email"` #añadir email privado en GitHub

    `git config --global user.name "username"` #añadir usuario de GitHub

    si añadió una clave SSH ejecute la siguiente línea

    `SSH -T git@github.com`

    Confirme fingerprint
